...
assume_role=$(echo $TERRAFORM_OUTPUTS | jq '.assume_role.value' | sed 's/"//g')
# echo $assume_role
...
cluster_role_arn=$(echo $TERRAFORM_OUTPUTS | jq '.eks_cluster_worker_iam_role_arn.value' | sed 's/"//g')
prometheus_image_repo=$(echo $TERRAFORM_OUTPUTS | jq '.prometheus_repository_v2.value.repository_url' | sed 's/"//g')
cluster_name=$(echo $TERRAFORM_OUTPUTS | jq  '.eks_cluster_id.value' | sed 's/"//g')
prometheus_thanos_storage_bucket_name=$(echo $TERRAFORM_OUTPUTS | jq '.prometheus_thanos_storage_bucket_name.value' | sed 's/"//g')
prometheus_thanos_storage_kms_key_id=$(echo $TERRAFORM_OUTPUTS | jq '.prometheus_thanos_storage_kms_key_id.value' | sed 's/"//g')
